...
```{r}
sample_means <- replicate(n_draws, mean(rbinom(sample_size, n_trials, pr_success)))
```
...
demo_sample_dist <- function(n, n_trials = 10, pr_success = 0.1, n_draws = 1000) {
  sample_means <- sample_dist(n, n_trials, pr_success, n_draws)
...
```{r}
demo_sample_dist(40, pr_success = 0.5)
```
